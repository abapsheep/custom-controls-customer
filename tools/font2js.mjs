...
const out = [
  ...
  "    metadata: METADATA,",
  "    // Load-bearing, and not optional: passing `metadata` does NOT stop UI5",
  "    // from fetching the metadata JSON. _loadFontMetadata defaults",
  "    //",
  '    //     metadataURI = fontURI + fontFamily + ".json"',
  "    //",
  "    // whenever it is undefined, and only ever reads the inline map on the",
  "    // branch it takes when metadataURI is FALSY. Left undefined, UI5 asks",
  "    // for the .json - against the data: URI above, which is not a directory",
  "    // - and the request dies on abap2UI5's CSP, whose connect-src is",
  '    // explicit and carries no data:. The collection then holds no icons at',
  "    // all: the names render, every glyph is blank, and the only trace is",
  '    // "An error occurred loading the font metadata for collection".',
  "    // Same in 1.71 and in 1.120+/2.x.",
  '    metadataURI: "",',
  "  };",
  "});",
  "",
